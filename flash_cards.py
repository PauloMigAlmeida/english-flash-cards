...
def filter_cards(filter_name):
    ...
    filters = {
        ...
        "prepositions": "where type = 2",
        "expressions":  "where type = 3",
        "known":        "where known = 1",
...
@app.route('/prepositions/<card_id>')
def prepositions(card_id=None):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    return memorize("prepositions", card_id)


@app.route('/expressions')
@app.route('/expressions/<card_id>')
def expressions(card_id=None):
    if not session.get('logged_in'):
        return redirect(url_for('login'))
    return memorize("expressions", card_id)


def memorize(card_type, card_id):
    if card_type == "general":
        type = 1
    elif card_type == "prepositions":
        type = 2
    elif card_type == "expressions":
        type = 3
    else:
        return redirect(url_for('cards'))
